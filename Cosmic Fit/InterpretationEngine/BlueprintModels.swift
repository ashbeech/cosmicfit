...
struct PaletteSection: Codable, Equatable {
    /// V4 neutral anchors (4 colours). Nil during legacy-to-V4 transition.
    let neutrals: [BlueprintColour]?          // D — V4 only
    /// Deterministic core colours (4 anchors in V4, 3–4 in legacy).
    ...
    let cluster: PaletteCluster?              // D — V4 only
    /// V4 derived variables. Nil for legacy blueprints.
    let variables: DerivedVariables?          // D — V4 only
    /// V4 secondary pull (adjacent family tilt). Nil if absent or legacy.
    let secondaryPull: PaletteFamily?         // D — V4 only
    /// V4 override flags (trace/debug metadata). Nil for legacy blueprints.
    let overrideFlags: OverrideFlags?         // D — V4 only
    /// Legacy swatch families. Nil in V4 blueprints.
    ...
    init(from decoder: Decoder) throws {
        ...
        secondaryPull = try container.decodeIfPresent(PaletteFamily.self, forKey: .secondaryPull)
...
enum ColourProvenance: Codable, Equatable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case let .v4Template(family, band, index):
            try container.encode(Kind.v4Template, forKey: .kind)
