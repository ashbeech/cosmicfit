...
class NatalChartViewController: UIViewController {
    ...
    private var aspectsSections: [[String: Any]] = []
    ...
    private func setupUI() {
        ...
        let image = renderer.image { ctx in
            ...
            // Draw house divisions (simplified)
            for i in 0..<12 {
                let angle = CGFloat(i) * .pi / 6
                let startX = rect.midX
                let startY = rect.midY
                let endX = rect.midX + cos(angle) * 150
                let endY = rect.midY + sin(angle) * 150
                
                ctx.cgContext.move(to: CGPoint(x: startX, y: startY))
                ctx.cgContext.addLine(to: CGPoint(x: endX, y: endY))
    ...
    private func processChartData() {
        ...
        // Process aspects
        if let aspects = chartData["aspects"] as? [[String: Any]] {
            aspectsSections = aspects
        }
    }
    ...
    @objc private func showInterpretation() {
        ...
        Ascendant, houses, and significant aspects based on your chart data.
...
extension NatalChartViewController: UITableViewDataSource, UITableViewDelegate {
    func numberOfSections(in tableView: UITableView) -> Int {
        return 4 // Planets, Houses, Angles, Aspects
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case 2: return anglesSections.count
        case 3: return min(aspectsSections.count, 10) // Limit to top 10 aspects
        default: return 0
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            ...
            if indexPath.row < planetSections.count {
                ...
                let house = planet["house"] as? Int ?? 0
        ...
        case 1:
            // Houses
            if indexPath.row < houseSections.count {
                let house = houseSections[indexPath.row]
                let houseNumber = house["house"] as? Int ?? 0
                let position = house["formattedPosition"] as? String ?? ""
        ...
        case 3:
            // Aspects
            if indexPath.row < aspectsSections.count {
                let aspect = aspectsSections[indexPath.row]
                let planet1 = aspect["planet1"] as? String ?? ""
                let planet2 = aspect["planet2"] as? String ?? ""
                let aspectType = aspect["aspectType"] as? String ?? ""
                let exactness = aspect["exactness"] as? Double ?? 0.0
                
                let exactnessText = String(format: "%.1f°", exactness)
                
                cell.configure(
                    title: "\(planet1) - \(planet2)",
                    detail: aspectType,
                    secondary: "Orb: \(exactnessText)"
                )
            }
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        case 1: return "Houses"
        case 2: return "Angles"
        case 3: return "Major Aspects"
        default: return nil
        }
    }
