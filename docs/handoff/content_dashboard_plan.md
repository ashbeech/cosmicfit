...
- **Editable content:** the **two rendered user-facing copy files** — `blueprint_narrative_cache.json` and `TarotCards.json` — **prose fields only** (no add/remove of structured list items). ⚠️ **`astrological_style_dataset.json` is NOT user-facing copy** (see below): it is an **upstream engine generation input** consumed by `DeterministicResolver` / `BlueprintTokenGenerator`, and is **deferred to v2**.

> **⚠️ astro-dataset is an engine input, not copy (v1 decision).** The dashboard's whole promise is a WYSIWYG loop: edit words → export → build → the words appear in the app. That holds for blueprint + TarotCards because they are the verbatim rendered strings. It does **not** hold for `astrological_style_dataset.json`: nothing in it is rendered directly (grep confirms its prose does not appear in the blueprint cache). An edit there changes the app only after the Style Guide **generation pipeline re-runs** (resolver → `backfill_narratives.py`), which also re-derives — and would churn — every blueprint override. So editing it behind this tool's edit→build loop would be a WYSIWYG lie *and* would collide with the reseed/reconcile policy. It is therefore **deferred to v2**, shown **read-only in the file picker** with a banner ("engine generation input — editing requires pipeline regeneration"). If it is added in v2 it needs a **regeneration-aware workflow** with an explicit "this does nothing until the pipeline re-runs" gate — arguably a different tool than this copy editor, not the drop-in-and-build path.
...
- **`blueprint_narrative_cache.json`** — **In scope, full (locked).** In today's V2 each of the 16 sections per cluster is an **object**, so editable prose = each section's `.text` (the 50–150-word paragraph) **+** its `.sectionIntro`, **plus** the cluster-level `closing` — ~33 editable fields/cluster × 576 = **19,008 fields**. Read-only: `coreFormula` (interpolated into `closing`) and every structured list (`rankedItems`/`tests`/`traps`). Facets: Venus / Moon / Element parsed from the cluster key.
...
- **`astrological_style_dataset.json`** — **DEFERRED to v2 (locked).** It is an engine generation input, not rendered copy (see the ⚠️ note under *Confirmed decisions*), so it is **not seeded** and appears **read-only** in the file picker with a banner. The field registry + migration are schema-compatible so it can be added later with **no DDL change**, but only via a regeneration-aware workflow. The blueprint + tarot files ship regardless.
...
**Hand-off checklist for Ash:** (1) Export in dashboard → download bundle (v1 = both blueprint files + `TarotCards.json` + `manifest.json`; **astro is not in the bundle** — it's deferred to v2); (2) **backup gate first** — `python3 tools/backup_content_sources.py backup --domain all --label export-vNN` (same rule the Python amend scripts enforce); (3) drop files at canonical paths — `data/style_guide/blueprint_narrative_cache.json` **and** `data/style_guide/blueprint_narrative_cache_sg4.json` (identical bytes, keeps the `inspector/` consumer in sync), and `Cosmic Fit/Resources/TarotCards.json` (real file, overwrite in place — the **first** drop also carries the one-time array-formatting normalization from `npm run normalize:tarot`); (4) Resources symlinks resolve at build — no Xcode change; (5) build & verify log `Loaded 576 archetype clusters (schema v2)`; (6) commit the files + manifest.
