...
- **OPEN DECISION — server-side entitlement (Play parity):** iOS gates **entirely client-side** (StoreKit `currentEntitlements`; see §G `app-store-notifications` = server-only, zero client refs). The server `subscription_status` ledger (migration `003`) is **audit-only scaffolding, not a live entitlement store**: nothing reads it (confirmed zero readers), the Apple webhook writes `expires_at` as `null` on every call ([`app-store-notifications/index.ts:115`](../../supabase/functions/app-store-notifications/index.ts#L115)), and a failed status upsert is non-fatal (logs, returns 200). Play RTDN parity forces a choice — **(a)** mirror iOS with client-only Play Billing gating, or **(b)** promote this ledger to a real server entitlement store. If (b): populate `expires_at`, add a reader/gate, and reconsider the non-fatal-200. **Not a bug today** — the monotonic guard `upsert_subscription_status` is `LANGUAGE sql`/`RETURNS void` with a declarative `WHERE EXCLUDED.last_event_signed_at > …` clause, so it does **not** carry the throwing plpgsql `boolean > integer` idiom of the other app's DI-11 (investigated 2026-07-15; does not apply here).
