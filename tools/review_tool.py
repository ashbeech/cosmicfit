...
A local-only web UI for reviewing AI-generated Style Guide narrative paragraphs
(stored in blueprint_narrative_cache.json — legacy filename).
Conforms to docs/narrative_review_tool_spec.md.

Usage (from repo root):
    python3 tools/review_tool.py [--cache blueprint_narrative_cache.json] [--port 8420]
...
# Overridden in __main__ after argparse (default lives under data/style_guide/).
CACHE_PATH = ""
...
if __name__ == "__main__":
    ...
    default_cache = str(repo_root / "data" / "style_guide" / "blueprint_narrative_cache.json")
    parser = argparse.ArgumentParser(description="Cosmic Fit Narrative Review Tool")
    parser.add_argument(
        "--cache",
        default=default_cache,
        help="Path to blueprint_narrative_cache.json (default: data/style_guide/)",
    )
