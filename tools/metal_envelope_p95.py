#!/usr/bin/env python3
"""Compute P95 metal tone baseline deviation from inspector cohort sample."""
...
# Metal envelope bounds — wider than contrast given metal baseline spread
HALF_SPAN_MIN = 0.28
HALF_SPAN_MAX = 0.44
MARGIN = 0.04
...
def main() -> int:
    ...
    def sample(user: dict, d: str) -> float | None:
        ...
        if not sp or "metalTone" not in sp:
            return None
        raw = float(payload["metalTone"])
        baseline = float(sp["metalTone"]["baseline"])
    ...
    half_span = round(min(HALF_SPAN_MAX, max(HALF_SPAN_MIN, round(p95 + MARGIN, 2))), 2)

    out = {
        "nSamples": len(deviations),
        "p95Deviation": round(p95, 4),
        "recommendedHalfSpan": half_span,
        "formula": f"clamp(round(P95 + {MARGIN}, 2), min={HALF_SPAN_MIN}, max={HALF_SPAN_MAX})",
        "bounds": {"min": HALF_SPAN_MIN, "max": HALF_SPAN_MAX, "margin": MARGIN},
    }
    out_path = ROOT / "docs" / "fixtures" / "metal_envelope_p95.json"
